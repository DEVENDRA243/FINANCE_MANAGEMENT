...
with st.sidebar:
    ...
    # Read API key from env (already updated in previous step)
    ...
    model_choice = st.selectbox("🤖 AI Model Version", 
                ...
                if len(found_cols) == 3:
                    ...
                    if st.button("🚀 Process and Save Transactions"):
                        with st.spinner("Categorizing and saving in batches..."):
                            ...
                            # Prepare for batch categorization (e.g., 20 at a time)
                            batch_size = 20
                            to_categorize = []
                            processed_data = []
                            
                            # Initial pass: identify what needs AI
                            ...
                            for i in range(0, len(to_categorize), batch_size):
                                ...
                                for res in results:
                                    ...
                                    if idx < len(batch):
                                        item = batch[idx]
                                        processed_data.append({
                                            "Date": str(item['date']), "Description": item['description'], "Amount": item['amount'],
...
with tab1:
    ...
    # Fetch fresh data every time tab 1 is rendered
    ...
    if not df.empty:
        ...
        with col_left:
            ...
            fig_pie = px.pie(spending_df, values='amount', names='category', title="Spending by Category", hole=0.4)
            st.plotly_chart(fig_pie, use_container_width=True)
            
        with col_right:
            # Monthly Trend
            df['Date'] = pd.to_datetime(df['date'], errors='coerce')
            # If parsing failed for some, try dayfirst
            mask = df['Date'].isna()
            if mask.any():
            ...
            fig_bar = px.bar(monthly_trend, x='Month', y='amount', title="Monthly Net Savings", color='amount', color_continuous_scale='RdYlGn')
            st.plotly_chart(fig_bar, use_container_width=True)
            
        # Transaction Table
        st.subheader("Recent Transactions")
        st.dataframe(df[['date', 'description', 'amount', 'category', 'confidence']].head(10), use_container_width=True)
...
with tab2:
    ...
    if prompt := st.chat_input("Ask about your finances..."):
        ...
        if api_key:
            try:
                # Use st.cache_resource to avoid re-creating the agent every turn
                agent = create_agent(api_key, model_choice)
                
                # Show spinner while agent thinks
                with st.spinner("FinGenius is analyzing your data..."):
                    response = agent.invoke({"messages": [HumanMessage(content=prompt)], "db": db})
                    last_msg = response["messages"][-1]
                    bot_reply = last_msg.content
                    
                    # Handle structured content if returned as list (common in some Gemini versions)
                    if isinstance(bot_reply, list):
                        ...
                        for part in bot_reply:
                            if isinstance(part, dict) and 'text' in part:
                                ...
                                parts.append(part)
                            elif hasattr(part, 'text'): # Some objects have .text
                                parts.append(part.text)
...
with tab3:
    ...
    with colA:
        ...
        if st.button("Generate 3 Months of Sample Data"):
            # Mock data generation similar to notebook
            from datetime import timedelta
            import random
            
            categories = ['Groceries', 'Dining', 'Transportation', 'Shopping', 'Entertainment', 'Utilities', 'Housing', 'Healthcare', 'Education', 'Income']
            data = []
            for i in range(220): # Generate 220 to match user's scale
